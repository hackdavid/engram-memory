...
class LocalEmbedding(BaseEmbedding):
    ...
    def __init__(self, model_name: str = "all-MiniLM-L6-v2") -> None:
        if sentence_transformers is None:
            raise ImportError(
                "sentence-transformers is required: pip install engram"
            )
        _silence_transformers_load_noise()
        print(
            "[engram] Loading SentenceTransformer model "
        ...
        print(
            f"[engram] SentenceTransformer ready: {model_name!r}, {dim} dimensions.",
            file=sys.stderr,
            flush=True,
        )
